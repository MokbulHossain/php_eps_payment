...
class EPS {
    ...
    function SignIn() {
        ...
        $curl = curl_init();
        $req_body = array("userName"=>$this->userName, "password"=>$this->password, "deviceTypeId"=>$this->deviceTypeId);
        $x_hash = $this->GenerateHash($this->userName,$this->hashkey);
    
        curl_setopt_array($curl, array(
        CURLOPT_URL => $this->baseUrl.'/v1/SignIn',
        ...
        */
        if ($responseCode >= 400) {
            die("HTTP Error in signing : ". $responseCode);
        }
        curl_close($curl);
    ...
    function GetToken() {
      ...
      $signin_response = $this->SignIn();
        if(!isset($signin_response->id)){
        ...
        $req_body = array(
            ...
            "name" => $signin_response->name ,
            "email" => $signin_response->email ,
            "phoneNumber" => $signin_response->phoneNumber,
            "role" => $signin_response->role,    
            "userName"=>$this->userName, 
            "password"=>$this->password
        );
        $x_hash = $this->GenerateHash($signin_response->id,$this->hashkey);
        $token = $signin_response->apiToken;
    ...
    function CreatePayment($payload = array()) {
        ...
        $getToken_response = $this->GetToken();
          if(!isset($getToken_response->jwtToken)){
          ...
          $token = $getToken_response->jwtToken;
